...
function JobDetailPage( { deleteJob } ) {
  ...
  const onDeleteClick = (jobId) => {
    const confirm = window.confirm('Are tou sure want to delete this Job?')
    if (!confirm) return;
    deleteJob(jobId);
    navigate('/jobs')
  }
...
const jobLoader = async ({params}) => {
  ...
  const fetchData = await fetch(`/api/jobs/${params.id}`)
  const fetchJob = await fetchData.json()
  return fetchJob
}
